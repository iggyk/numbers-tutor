export const create = (elementName, id, content, props, clickHandler) => {
    ...
    if (clickHandler) result.addEventListener("click", clickHandler);
    return result;
}

export const createTracker = () => {
    const container = create("div", "tracker", null, { className: "closed"});
    const header = create("div","header", "Tracker", null, () => {
        container.className = container.className === "closed" ? "open" : "closed";
    });
    const report = create("div", "report");
    container.appendChild(header);
    container.appendChild(report);
    const track = (record) => {
        report.innerHTML = record.map(x => `<div>${x.number}: ${x.attempts}</div>`).join("");
    }
    return {
        container,
        track
